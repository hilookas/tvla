...
class Pi0Model(Pi0PreTrainedModel):
    def __init__(self, config):
        ...
        vlm_config_hf.text_config.hidden_size = GEMMA_SIZE[
            self.config.paligemma_variant
        ]["width"]
        vlm_config_hf.text_config.intermediate_size = GEMMA_SIZE[
            self.config.paligemma_variant
        ]["mlp_dim"]
        ...
        action_expert_config_hf = GemmaConfig(
            ...
            torch_dtype="float32",
            adarms_cond_dim=GEMMA_SIZE[self.config.action_expert_variant]["width"]
            if self.config.pi05
            else None,
            use_norm=False,
        )
        ...
        if self.config.pi05:
            self.time_mlp_in = nn.Linear(
                action_expert_config_hf.hidden_size, action_expert_config_hf.hidden_size
            )
            ...
            self.action_time_mlp_in = nn.Linear(
                2 * action_expert_config_hf.hidden_size,
                action_expert_config_hf.hidden_size,
            )
